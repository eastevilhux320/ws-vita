...
class LaunchViewModel(application: Application) : BizcoreViewModel(application) {
    ...
    fun protocolResult(flag: Boolean) {
        ...
        if (flag) {
            ...
            /**
             * 记录用户同意状态。
             * * 这里的 key 必须与 [LaunchViewModel.disposeProtocol] 方法中生成的 key 保持一致。
             */
    ...
    private fun emitSplash(event: LaunchEvent) {
        ...
        viewModelScope.launch {
            ...
            StartupConfigLocator.instance.dispatchAction(StartupScope.STARTUP_SCOPE_APPBEFORE);
        }
    }
