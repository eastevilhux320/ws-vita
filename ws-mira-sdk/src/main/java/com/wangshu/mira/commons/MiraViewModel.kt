...
abstract class MiraViewModel(application: Application) : NavigationViewModel(application) {

    /**
     * 【核心通道】
     */
    private val _commonDataFlow = MutableSharedFlow<WsDataEvent<*>>()
    val commonDataFlow = _commonDataFlow.asSharedFlow()

    /**
     * 玄映SDK配置
     */
    val config = MutableLiveData<MiraConfig>();

    override fun initModel() {
        super.initModel()
        config.value = MiraConfigure.instance.getConfig();
    }
     ...
     *
     * 核心逻辑：
     * 1. 自动将原始 Int 包装为 [WsDataEvent.IntEvent]，彻底消除硬编码 String 标识。
    ...
    fun sendEmpty() {
        viewModelScope.launch {
            _commonDataFlow.emit(WsDataEvent.Empty())
        }
    }
     ...
     *
     * 【执行流程】：
     * 1. 类型检查：通过 `when` 表达式强制匹配预定义的 [WsDataEvent] 类型 [cite: 1]。
    ...
    private fun <T : Any> sendCommonData(value: T) {
        viewModelScope.launch {
            val event = when (value) {
                is Int -> WsDataEvent.IntEvent(value)
                is Long -> WsDataEvent.LongEvent(value)
                is Double -> WsDataEvent.DoubleEvent(value)
                is String -> WsDataEvent.StringEvent(value)
